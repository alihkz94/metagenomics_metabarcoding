...
#   - detailed_results/: Unknown-labeled sequences, multiple alignments, CSV details #
...
def check_blast_needed(input_chimeras_dir, output_dir):
    ...
    # Collect input files (now plain FASTA names; keep backward compatibility with .chimeras)
    fasta_extensions = [".fasta", ".fa", ".fas"]
    chimeric_files = []

    for ext in fasta_extensions:
        chimeric_files.extend([f for f in os.listdir(input_chimeras_dir) if f.endswith(ext)])

    if not chimeric_files:
        logger.error(f"No FASTA files found in {input_chimeras_dir} (expected *.fasta/*.fa/*.fas)")
    ...
    for chim_file in chimeric_files:
        base_name = os.path.splitext(chim_file)[0]  # e.g., "ERR6454463" or "ERR6454463.chimeras"
...
def generate_blasch_readme(
    ...
    # Count input files (plain FASTA names; backward compatible with .chimeras)
    fasta_extensions = [".fasta", ".fa", ".fas"]
    chimeric_files = []
    for ext in fasta_extensions:
        chimeric_files.extend(glob.glob(os.path.join(input_chimeras_dir, f"*{ext}")))
    ...
    unknown_count = len([f for f in os.listdir(detailed_results_dir) if f.endswith('_unknown.fasta')]) if os.path.exists(detailed_results_dir) else 0
    ...
    try:
        with open(readme_path, 'w') as f:
            ...
            f.write(f"Number of input FASTA files processed: {len(chimeric_files)}\n")
            ...
            f.write(f"Unknown sequences: {unknown_count} files\n")
            ...
            f.write("#   *_unknown.fasta           = Sequences classified as unknown (previously chimeric)\n")
            ...
            f.write("2. Sequences with only self-hits are labeled as Unknown (previously Chimeric)\n")
            ...
            f.write("5. All other sequences → Borderline or Unknown\n\n")
...
def run_single_blast(args_tuple):
    """
    Worker function to run a single BLAST analysis.
    Designed for parallel execution via multiprocessing.
    
    Args:
        args_tuple: (chimera_file, base_name, self_db, reference_db_prefix, 
                     xml_file, threads_per_job)
    
    Returns:
        (success: bool, base_name: str, message: str)
    """
    chimera_file, base_name, self_db, reference_db_prefix, xml_file, threads_per_job = args_tuple
    
    try:
        # Validate the chimera file
        if not validate_fasta_with_biopython(chimera_file):
            return (False, base_name, f"Invalid chimeras file: {chimera_file}")
        
        # Check self database exists
        if not check_blast_db_exists(self_db):
            return (False, base_name, f"Self DB not found: {self_db}")
        ...
        if reference_db_prefix:
            ...
            db_info = f"reference + self DB"
        else:
            cmd.extend(["-db", self_db])
            db_info = f"self DB only"
        
        logger.info(f"[{base_name}] Starting BLAST ({db_info}, {threads_per_job} threads)")
        
        # Run BLAST
        subprocess.run(cmd, check=True, capture_output=True, text=True)
        
        logger.info(f"[{base_name}] BLAST completed successfully")
        return (True, base_name, "Success")
        
    except subprocess.CalledProcessError as e:
        error_msg = f"BLAST failed: {e.stderr if e.stderr else str(e)}"
...
def run_blast_analysis(
    ...
    OPTIMIZED FOR HPC: Runs multiple BLAST jobs in parallel.
    
    Args:
        max_parallel_blast: Maximum number of BLAST jobs to run simultaneously.
                           Each job will use (threads / max_parallel_blast) CPUs.
    
    Exit on BLAST failure.
    ...
    # Collect input files (plain FASTA names; keep backward compatibility with .chimeras)
    fasta_extensions = [".fasta", ".fa", ".fas", ".fna"]
    chimeric_files = []
    ...
    if not chimeric_files:
        logger.error(f"No FASTA files found in {input_chimeras_dir} with extensions {fasta_extensions}.")
    ...
    # ========================================================================
    # HPC OPTIMIZATION: Parallel BLAST execution
    # ========================================================================
    
    # Calculate optimal parallelization strategy
    num_files = len(chimeric_files)
    
    # Determine how many BLAST jobs to run in parallel
    if num_files <= max_parallel_blast:
        # If we have fewer files than max parallel jobs, run all simultaneously
        actual_parallel_jobs = num_files
        threads_per_job = max(1, threads // actual_parallel_jobs)
    else:
        # Run max_parallel_blast jobs at a time
        actual_parallel_jobs = max_parallel_blast
        threads_per_job = max(1, threads // max_parallel_blast)
    
    logger.info(f"""\n{'='*70}
HPC BLAST Optimization Strategy:
  Total files to process: {num_files}
  Parallel BLAST jobs: {actual_parallel_jobs}
  Threads per BLAST job: {threads_per_job}
  Total threads used: {actual_parallel_jobs * threads_per_job}
{'='*70}\n""")
    
    # Prepare arguments for all BLAST jobs
    blast_args = []
    for chim_file in chimeric_files:
        chimera_file = os.path.join(input_chimeras_dir, chim_file)
        base_name = os.path.splitext(chim_file)[0]
        raw_base = base_name.replace(".chimeras", "")
        for suffix in ("_nonchimeric", ".nonchimeric", "-nonchimeric", "_chimeric", ".chimeric", "-chimeric"):
            if raw_base.endswith(suffix):
                raw_base = raw_base[:-len(suffix)]
                break

        # Fallback for naming mismatches: if exact DB name is missing,
        # use any existing self-DB whose sample name prefixes the input file base.
        expected_prefix = os.path.join(self_db_dir, raw_base, raw_base)
        if not check_blast_db_exists(expected_prefix):
            try:
                for d in sorted(os.listdir(self_db_dir)):
                    db_prefix = os.path.join(self_db_dir, d, d)
                    if check_blast_db_exists(db_prefix) and (raw_base == d or raw_base.startswith(f"{d}_")):
                        raw_base = d
                        break
            except Exception:
                pass
    ...
    # Run BLAST jobs in parallel using multiprocessing
    logger.info(f"Starting parallel BLAST execution with {actual_parallel_jobs} workers...\n")
    
    start_blast_time = time.time()
    
    with multiprocessing.Pool(processes=actual_parallel_jobs) as pool:
        results = pool.map(run_single_blast, blast_args)
    
    blast_duration = time.time() - start_blast_time
    
    # Check results
    failed_jobs = [(base_name, msg) for success, base_name, msg in results if not success]
    successful_jobs = [base_name for success, base_name, msg in results if success]
    
    logger.info(f"""\n{'='*70}
BLAST Execution Summary:
  Total jobs: {len(results)}
  Successful: {len(successful_jobs)}
  Failed: {len(failed_jobs)}
  Total time: {blast_duration:.2f} seconds
  Average time per file: {blast_duration/len(results):.2f} seconds
{'='*70}\n""")
    
    if failed_jobs:
        logger.error("The following BLAST jobs failed:")
        for base_name, msg in failed_jobs:
            logger.error(f"  - {base_name}: {msg}")
        sys.exit(1)
    
    logger.info("All BLAST analyses completed successfully.\n")
...
def parse_blast_results(args):
    ...
    # We look for chimeras FASTA file with various extensions (now plain names, still accept .chimeras)
    fasta_extensions = [".fasta", ".fa", ".fas", ".fna"]
    fasta_path = None
            ...
            for record in records:
                ...
                if is_mult:
                    ...
                    for i, hit in enumerate(hits_info, 1):
                        ...
                        # Re-label chimeric classifications as unknown in reports
                        classification_label = "unknown" if classification == "chimeric" else classification
                        details.append([
                            qid,
                            f"{hit['coverage']:.2f}",
                            f"{hit['identity']:.2f}",
    ...
    if chimeric:
        ch_file = os.path.join(detailed_results_dir, f"{trimmed_base_name}_unknown.fasta")
...
def generate_report(
    ...
    counts = {
        ...
        "Unknown Sequences": len(all_chimeric),
...
def main():
    ...
    parser.add_argument("--threads", type=int, default=8,
                        help="Total number of CPU threads available for BLAST.")
    parser.add_argument("--max_parallel_blast", type=int, default=8,
                        help="Maximum number of BLAST jobs to run in parallel (default: 8). "
                             "Each job will use threads/max_parallel_blast CPUs.")
    ...
    run_blast_analysis(
        ...
        max_parallel_blast=args.max_parallel_blast
    )

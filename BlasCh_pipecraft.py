...
def analyze_blast_hits(blast_record, query_id):
    ...
    # forced-chimeric if multiple HSPs in the first non-self
...
def classify_sequence(
    ...
    if any(h["force_chimeric"] for h in hits_info):
        return "chimeric", "Multiple HSPs in first non-self hit"
        ...
        with open(xml_path) as handle:
            ...
            for record in records:
                ...
                for i, hit in enumerate(hits_info, 1):
                    row_class = "multiple_alignment" if is_mult else classification
                    details.append([
                        qid,
                        f"{hit['coverage']:.2f}",
                        f"{hit['identity']:.2f}",
...
def main():
    ...
    parser.add_argument("--reference_db", default="",
    ...
    parser.add_argument("--output_dir", default="./rescued_reads",
                        help="Directory where all results are written (default=./rescued_reads).")

    parser.add_argument("--threads", type=int, default=8,
                        help="Number of CPU threads for BLAST (default=8).")
